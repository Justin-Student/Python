...
class THREDDS_Models:
    ...
    * Reference Catalog: http://thredds-jetstream.unidata.ucar.edu/thredds/idd/forecastModels.html
    ...
    def change_run_time(self,year,month,day):
        '''
    ...
    def get_model_data(self,init_hour,model,prod,*argv):
        ...
        now = datetime(self.today_year,self.today_month,self.today_day,int(init_hour[0:2]))
    ...
    #---------------------------------------------------------------------------------------------------
        ...
        if contourfill == True:
            if data.ndim ==4:
                ...
                cs2 = ax.contourf(lons, lats, data[time_index,data.variables["isobaric"][:].tolist().index(50000),:,:], self.clevs, cmap=cmap,
                          transform=datacrs)
        ...
        if contourfill == False:
            if data.ndim ==4:
                cs2 = ax.contour(lons, lats, data[time_index,data.variables["isobaric"][:].tolist().index(50000),:,:], self.clevs, colors=self.colors,
                      transform=datacrs)
